...
from future_view_prediction_w_action_dataset import get_future_view_prediction_w_action_data_loader
...
class BaseTrainer:
    def __init__(self,
        ...
        #
        if self.model_class.name == "mllm":
            if train_cfg.freeze_lm_head:
                self.log.info("Freezing lm head")
                self.model.model.paligemma.paligemma.language_model.lm_head = None # remove VLM lm head
                # for param in self.model.model.paligemma.paligemma.language_model.model.embed_tokens.parameters():
                #     param.requires_grad = False
            if train_cfg.freeze_vlm:
                # freeze VLM
                self.log.info("Freezing VLM")
                for param in self.model.model.paligemma.paligemma.language_model.model.parameters():
                    param.requires_grad = False
        ...
        # Dataloaders
        self.train_dataloader = get_future_view_prediction_w_action_data_loader(
            dataset_path="/vast/bc4227/datasets/bridge_processed",
        ...
        if self.model_class.name == "mllm":
            vlm_parameters = filter(lambda p: p.requires_grad, base_model.model.paligemma.paligemma.parameters())
    ...
    def _initialize_model(self, train_cfg, model_class):
        ...
        if train_cfg.load_from_checkpoint is None:
            self.model = model_class(config=self.model_cfg)
        else:

...
from src.model.mmmvla.transformer_encoder import Qwen2Encoder
from lerobot.common.constants import ACTION, OBS_ROBOT
from lerobot.common.policies.normalize import Normalize, Unnormalize
from src.model.mmmvla.configuration_mmmvla import MMMVLAConfig
from src.model.mmmvla.mllm import (PaliGemmaModel, PaliGemmaConfig)
...
class MMMVLAPolicy(PreTrainedPolicy):
    """Wrapper class around PI0FlowMatching model to train and run inference within LeRobot."""

    config_class = MMMVLAConfig
    name = "mmmvla"
    ...
    def __init__(
        ...
        self.num_metaqueries = config.num_metaqueries 
        """
        self.pad_token_id = getattr(
            self.language_tokenizer, "tokenizer", self.language_tokenizer
        ).pad_token_id  # pad_token_id of the tokenizer
        if config.num_metaqueries > 0:
            tokenizer = getattr(self.language_tokenizer, "tokenizer", self.language_tokenizer)
            ...
            print(f"BOI token ID: {self.boi_token_id}, EOI token ID: {self.eoi_token_id}")
            self.metaquery_token_ids = [tokenizer.convert_tokens_to_ids(f"<img{i}>") for i in range(self.num_metaqueries)]
        """
        self.reset()
    ...
    def select_action(self, batch: dict[str, Tensor], noise: Tensor | None = None) -> Tensor:
        ...
        if len(self._action_queue) == 0:
            ...
            actions = actions[:, :, :original_action_dim] # (batch_size, 50, action_dim)
...
class PI0FlowMatching(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.connector_out_dim = self.action_expert.hidden_size # 1024
        ...
        encoder = Qwen2Encoder(
            Qwen2Config(
                ...
                num_hidden_layers=self.connector_num_hidden_layers,  # 24
    ...
    def embed_prefix(
        ...
        att_masks += [1] * self.config.num_metaqueries
    ...
    def embed_suffix(self, state, noisy_actions, timestep):
        ...
        att_masks += [1] * self.config.n_action_steps
    ...
    def forward(
        ...
        prompt_embs, prefix_q, prefix_k, prefix_v = self.paligemma.forward(
            attention_mask=prefix_att_2d_masks,
            position_ids=prefix_position_ids,
            inputs_emb=prefix_embs,
        )

        prefix_pad_masks = prefix_pad_masks[:, :-self.num_metaqueries] 
        prefix_att_masks = prefix_att_masks[:, :-self.num_metaqueries]

        pad_masks = torch.cat([prefix_pad_masks, suffix_pad_masks], dim=1)
        att_masks = torch.cat([prefix_att_masks, suffix_att_masks], dim=1)

        self_att_2d_masks = make_att_2d_masks(pad_masks, att_masks)
        self_position_ids = torch.cumsum(pad_masks, dim=1) - 1

        prompt_embs = self.connector(prompt_embs)
        suffix_len = suffix_pad_masks.shape[1] # 1 + 50
        batch_size = prompt_embs.shape[0] # B
        prompt_len = prompt_embs.shape[1] # 18*18
        ...
        prompt_position_ids = torch.arange(prompt_len, device=prompt_embs.device).unsqueeze(0).expand(batch_size, -1)

        prompt_pad_masks = torch.ones((batch_size, prompt_len), dtype=torch.bool, device=prompt_embs.device)
        prompt_att_masks = torch.zeros((batch_size, prompt_len), dtype=torch.bool, device=prompt_embs.device)
        ppad_masks = torch.cat([prompt_pad_masks, suffix_pad_masks], dim=1)
        aatt_masks = torch.cat([prompt_att_masks, suffix_att_masks], dim=1)
        sself_att_2d_masks = make_att_2d_masks(ppad_masks, aatt_masks)
        pposition_ids = torch.cumsum(ppad_masks, dim=1) - 1

        cross_attention_mask = torch.ones((batch_size, suffix_len, prompt_len), dtype=torch.bool, device=suffix_att_2d_masks.device)
        ...
        suffix_out, _ = self.action_expert.forward(
            ...
            prefix_k=prefix_k,
            prefix_v=prefix_v,
            self_attention_mask=self_att_2d_masks,
            self_position_ids=self_position_ids,
            cross_attention_mask=cross_attention_mask,
            suffix_position_ids=suffix_position_ids,
            prompt_position_ids=prompt_position_ids,
            pposition_ids=pposition_ids,
            sself_att_2d_masks=sself_att_2d_masks,
    ...
    def sample_actions(self, images, img_masks, lang_tokens, lang_masks, state, noise=None) -> Tensor:
        ...
        prompt_embeds, prefix_q, prefix_k, prefix_v = self.paligemma.forward(
            attention_mask=prefix_att_2d_masks,
            position_ids=prefix_position_ids,
            inputs_emb=prefix_embs,
        )
        ...
        prompt_len = prompt_embeds.shape[1] # 64
        prompt_position_ids = torch.arange(prompt_len, device=prompt_embeds.device).unsqueeze(0).expand(batch_size, -1)

        prompt_pad_masks = torch.ones((batch_size, prompt_len), dtype=torch.bool, device=prompt_embeds.device)
        prompt_att_masks = torch.zeros((batch_size, prompt_len), dtype=torch.bool, device=prompt_embeds.device)

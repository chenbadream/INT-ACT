...
class ActionExpertModel(PreTrainedModel):
    ...
    def forward_attn_layer(
        ...
        model_layers,
        layer_idx,
        prefix_q: Optional[torch.FloatTensor],
        prefix_k: Optional[torch.FloatTensor],
        prefix_v: Optional[torch.FloatTensor],
        suffix_embeds,
        position_ids,
        attention_mask,
        batch_size,
        ...
        query_states.append(prefix_q)
        key_states.append(prefix_k)
        value_states.append(prefix_v)
        ...
        att_output = att_output[:, -suffix_embeds.shape[1]:, :]  # Keep only the suffix part
        
        return att_output
    
    def forward_attn_layer_with_prompt(
        self,
        model_layers,
        layer_idx,
        prompt_embeds,
        ...
        # Apply layer normalization
        hidden_states = layer.input_layernorm(suffix_embeds)
        prompt_hidden_states = layer.input_layernorm(prompt_embeds)
        
        # Compute Q, K, V for suffix embeddings
        input_shape = hidden_states.shape[:-1]
        hidden_shape = (*input_shape, -1, layer.self_attn.head_dim)

        prompt_input_shape = prompt_hidden_states.shape[:-1]
        prompt_hidden_shape = (*prompt_input_shape, -1, layer.self_attn.head_dim)

        prompt_hidden_states = prompt_hidden_states.to(dtype=layer.self_attn.q_proj.weight.dtype)
        prompt_query_state = layer.self_attn.q_proj(prompt_hidden_states).view(prompt_hidden_shape)
        prompt_key_state = layer.self_attn.k_proj(prompt_hidden_states).view(prompt_hidden_shape)
        prompt_value_state = layer.self_attn.v_proj(prompt_hidden_states).view(prompt_hidden_shape)

        query_states.append(prompt_query_state)
        key_states.append(prompt_key_state)
        value_states.append(prompt_value_state)
    ...
    def forward_cross_attn_layer(
        ...
        if use_cache:
            if fill_kv_cache:
                ...
                # Apply RoPE to keys
                key_states = apply_rope(key_states, prompt_position_ids)
    ...
    def forward(
        ...
        prefix_q: Optional[List[torch.FloatTensor]] = None,
        prefix_k: Optional[List[torch.FloatTensor]] = None,
        prefix_v: Optional[List[torch.FloatTensor]] = None,
        self_attention_mask: Optional[torch.Tensor] = None,
        self_position_ids: Optional[torch.LongTensor] = None,
        cross_attention_mask: Optional[torch.Tensor] = None,
        suffix_position_ids: Optional[torch.LongTensor] = None,
        prompt_position_ids: Optional[torch.LongTensor] = None,
        pposition_ids: Optional[torch.LongTensor] = None,
        sself_att_2d_masks: Optional[torch.Tensor] = None,
        ...
        for layer_idx in range(num_layers):
            ...
            if use_self_attention:
                ...
                """
                att_output = self.forward_attn_layer_with_prompt(
                    model.layers,
                    layer_idx,
                    prompt_embeds,
                    suffix_embeds,
